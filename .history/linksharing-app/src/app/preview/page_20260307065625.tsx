    ...
    return (
      <main className="min-h-screen bg-[#FAFAFA] px-4 py-6">
        <div className="mx-auto max-w-[1392px] rounded-xl bg-white p-6 shadow-sm">
          <p className="text-sm text-[#737373]">You need to be logged in to view this page.</p>
        </div>
      </main>
    );
  }
      ...
      <div className="mx-auto max-w-[1392px]">
        <header className="rounded-xl bg-white p-4 shadow-sm md:p-6">
          <div className="flex items-center justify-between gap-4">
            ...
            {username ? <CopyProfileLinkButton username={username} label="Share Link" /> : null}
          </div>
        </header>
          ...
          <div className="relative md:-mt-[220px] md:px-4 md:pb-16">
            <div className="mx-auto w-full max-w-[349px] rounded-[24px] bg-white px-6 pb-12 pt-12 shadow-[0_0_32px_rgba(0,0,0,0.08)] md:px-10">
              <div className="text-center">
                ...
                {profile?.email ? (
                  <p className="mt-2 text-[16px] text-[#737373]">{profile.email}</p>
                ) : username ? (
                  <p className="mt-2 text-[16px] text-[#737373]">@{username}</p>
                ) : null}
              </div>
              ...
              <div className="mt-14 space-y-4">
                {links && links.length > 0 ? (
                  links.map((link) => {
                    ...
                    return (
                      <a
                        ...
                        style={{
                          backgroundColor: platform?.backgroundColor || "#1A1A1A",
                  ...
                  <div className="rounded-xl bg-[#FAFAFA] px-6 py-10 text-center">
                    <p className="text-sm text-[#737373]">No links have been added yet.</p>
                  </div>
                )}
              </div>
            </div>
